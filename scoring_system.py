...
def initialize_teams():
    if messagebox.askyesno("Confirm Initialization",
                            f"This will reset all existing team data and member data. Do you want to initialize {NUM_TEAMS} empty teams?"):
        global teams_data
        teams_data = {}
        ...
        status_label.config(text=f"{NUM_TEAMS} default teams initialized.", fg="blue")
        save_data()
...
def manage_teams_popup():
    ...
    if not team_names:
        tk.Label(popup, text="No teams initialized yet. Please initialize teams first.").pack()
        return
...
def select_event_popup():
    ...
    center_window(popup, 600) # Removed fixed height
    ...
    tk.Button(popup, text="Confirm Selection", command=confirm_event_selection).pack(pady=10)


def record_team_score_popup():
    popup = tk.Toplevel(root)
    popup.title("Record Team Scores")
    center_window(popup, 600) # Removed fixed height, adjusted width slightly
    ...
    if not selected_event_for_tournament:
        ...
        return
    if not teams_data:
        tk.Label(popup, text="No teams initialized. Please initialize teams first.").pack(pady=20)
        return
    ...
    def save_team_score():
        ...
        if wins_entry is None and points_entry is None: # This should ideally not happen if update_input_fields ran
            score_msg_label.config(text="Error: Input fields not initialized. Please re-open.", fg="red")
            return
    ...
    tk.Button(popup, text="Save Team Score", command=save_team_score).pack(pady=10)


def show_leaderboard_popup():
    popup = tk.Toplevel(root)
    popup.title("Overall Leaderboard")
    center_window(popup, 500) # Removed fixed height
    ...
    # Headers
    tk.Label(leaderboard_frame, text="Rank", font=("Arial", 10, "bold"), width=5, anchor="w").grid(row=0, column=0)
    tk.Label(leaderboard_frame, text="Team Name", font=("Arial", 10, "bold"), width=20, anchor="w").grid(row=0, column=1)
    tk.Label(leaderboard_frame, text="Score", font=("Arial", 10, "bold"), width=15, anchor="e").grid(row=0, column=2)
    ...
    if selected_event_for_tournament and event_details[selected_event_for_tournament]["type"] == "Tournament":
        tk.Label(leaderboard_frame, text="W/L", font=("Arial", 10, "bold"), width=8, anchor="e").grid(row=0, column=3)
    ...
    for team_name, data in sorted_teams:
        ...
        tk.Label(leaderboard_frame, text=str(display_rank_counter), width=5, anchor="w").grid(row=display_rank_counter, column=0)
        tk.Label(leaderboard_frame, text=team_name, width=20, anchor="w").grid(row=display_rank_counter, column=1)
        tk.Label(leaderboard_frame, text=str(team_score), width=15, anchor="e").grid(row=display_rank_counter, column=2)
        if display_text_extra: # Only display the extra column if it's applicable
            tk.Label(leaderboard_frame, text=display_text_extra, width=8, anchor="e").grid(row=display_rank_counter, column=3)
        display_rank_counter += 1
...
tk.Button(root, text="Initialize 5 Teams", command=initialize_teams).pack(pady=5)
tk.Button(root, text="Manage Teams & Members", command=manage_teams_popup).pack(pady=5)
tk.Button(root, text="View & Select Event", command=select_event_popup).pack(pady=5) # New button
tk.Button(root, text="Record Team Scores", command=record_team_score_popup).pack(pady=5)
tk.Button(root, text="Show Overall Leaderboard", command=show_leaderboard_popup).pack(pady=5)
tk.Button(root, text="Save Current Data", command=save_data).pack(pady=5)
tk.Button(root, text="Exit Application", command=on_closing).pack(pady=10)
